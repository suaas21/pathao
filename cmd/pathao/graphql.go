...
import (
	"github.com/suaas21/pathao/infra/sentry"
	"github.com/suaas21/pathao/repo/user"
	"log"
	"net/http"
	"os"
	"os/signal"
	...
	"github.com/suaas21/pathao/api"
	"github.com/suaas21/pathao/config"
	infraArango "github.com/suaas21/pathao/infra/arango"
...
func serve(cmd *cobra.Command, args []string) error {
	...
	userRepo := user.NewArangoUserRepository(client, lgr)

	bookAuthorService := service.NewUser(userRepo, lgr)
...
func startApiServer(cfg *config.Application, bookAuthorService *service.User, lgr logger.StructLogger) error {
	baCtrl := api.NewUserController(*bookAuthorService, lgr)
